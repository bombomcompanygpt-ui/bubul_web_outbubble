...
const Profile: React.FC = () => {
  ...
  // --- LOGIKA GAME BUBUL LEADERBOARD SYNC ---
  ...
  useEffect(() => {
    ...
    const calculateUserRank = (firestoreList: any[]) => {
      ...
      const userIndex = sorted.findIndex(
        (item) => item.name.toLowerCase() === (user.username || "").toLowerCase()
      );
      ...
      if (userIndex !== -1) {
        setUserGameRank({
          ...
          mode: sorted[userIndex].mode || 'Master Quiz',
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-10 py-6 md:py-8 px-4 sm:px-6 lg:px-8 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          ...
          {/* Peringkat Game Bubul 🫧 */}
          <section className="bg-gradient-to-br from-cyan-600 via-blue-700 to-[#031466] p-6 md:p-10 rounded-[35px] md:rounded-[50px] shadow-xl md:shadow-2xl text-white relative overflow-hidden border-b-[8px] md:border-b-[12px] border-cyan-400 group">
             <div className="absolute top-[-20px] right-[-20px] opacity-20 group-hover:scale-110 transition-transform duration-700">
                <Gamepad2 size={130} className="md:w-[160px] md:h-[160px]" />
             </div>
             
             <div className="relative z-10 space-y-5">
               <div className="flex items-center justify-between">
                 <h3 className="text-xl md:text-2xl font-black flex items-center gap-2 md:gap-3 uppercase tracking-tighter text-cyan-200">
                   <Gamepad2 size={26} className="text-cyan-300" /> Game Bubul 🫧
                 </h3>
                 <span className="px-3 py-1 bg-cyan-400/20 border border-cyan-300/30 rounded-full text-[10px] font-black uppercase text-cyan-200 flex items-center gap-1">
                   <Sparkles size={12} /> Live Rank
                 </span>
               </div>

               <div className="bg-white/10 backdrop-blur-md p-5 md:p-6 rounded-[28px] border border-white/20 text-center space-y-3">
                 <p className="text-xs font-black uppercase tracking-widest text-cyan-200">Peringkat Papan Skor Saya</p>
                 
                 {userGameRank.rank ? (
                   <div>
                     <div className="text-4xl sm:text-5xl font-black text-amber-300 drop-shadow-lg flex items-center justify-center gap-2">
                       <Trophy size={36} className="text-amber-400" /> #{userGameRank.rank}
                     </div>
                     <p className="text-xs font-bold text-cyan-100 mt-1">
                       dari {userGameRank.totalPlayers} Pemain Terdaftar
                     </p>
                   </div>
                 ) : (
                   <div>
                     <div className="text-2xl font-black text-slate-200">Belum Ada Skor</div>
                     <p className="text-xs font-bold text-cyan-100/80 mt-1">
                       Mainkan game untuk masuk ke Papan Peringkat!
                     </p>
                   </div>
                 )}

                 {userGameRank.highScore > 0 && (
                   <div className="pt-2 border-t border-white/10 flex justify-around text-xs font-bold text-white">
                     <div>
                       <span className="block text-[10px] text-cyan-200 uppercase font-extrabold">Skor Tertinggi</span>
                       <span className="text-base font-black text-amber-300">{userGameRank.highScore} Poin</span>
                     </div>
                     <div>
                       <span className="block text-[10px] text-cyan-200 uppercase font-extrabold">Mode Game</span>
                       <span className="text-xs font-black text-white">{userGameRank.mode}</span>
                     </div>
                   </div>
                 )}
               </div>
               ...
               <button
                 ...
                 className="w-full py-3.5 px-6 bg-amber-400 hover:bg-amber-300 text-slate-900 font-black rounded-2xl text-xs sm:text-sm uppercase tracking-wider shadow-lg hover:shadow-cyan-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95"
               >
                 <span>Main Game Bubul Sekarang</span>
                 <ArrowRight size={16} />
               </button>
             </div>
          </section>
